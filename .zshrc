...
# ./install flags: parsed from the script's own usage comment (#   --flag  text)
_dotfiles_install() {
  if [[ -f $words[1] ]]; then
    local -a flags
    flags=(${(f)"$(sed -nE 's/^#   (--[a-z-]+) +(.*)/\1[\2]/p' $words[1])"})
    _arguments $flags
  else
    _default
  fi
...
# Tool completions generated by install (kubectl/fzf, *.zsh).
