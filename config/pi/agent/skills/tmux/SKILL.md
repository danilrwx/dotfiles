...
- **dlv (Go debugger)**: `tmux ... send-keys -- 'dlv debug ./main.go' Enter`; set breakpoint `tmux ... send-keys -- 'b main.main' Enter`; continue `tmux ... send-keys -- 'continue' Enter`; step `tmux ... send-keys -- 'step' Enter`; next `tmux ... send-keys -- 'next' Enter`; print variables `tmux ... send-keys -- 'print varName' Enter`; stack trace `tmux ... send-keys -- 'bt' Enter`; exit via `quit` then confirm `y`.
