...
Сейчас в `pi-core` есть такие базовые skills:
...
- **web-search** — быстрый веб-поиск;
- **tmux** — управление интерактивными CLI-процессами в tmux;
- **session-query** — запрос данных из прошлых сессий Pi через session file;
- **skill-creator** — шаблон/гайд для создания новых skills;
- **caveman** — ультра-краткий режим общения для экономии токенов.

### Основные extensions

Сейчас в `pi-core` есть такие основные extensions:

- **permission** — встроенный permission layer для действий агента;
...
- **session-query** — чтение контекста из прошлых сессий;
- **context** — вспомогательная работа с контекстом и его состоянием;
- **model-price** — информация по стоимости моделей;
- **golangci-lint** — авто-запуск `golangci-lint run --fix` для изменённых Go-файлов;
- **jina-web** — web search и fetch content через Jina.

### Дополнительно: `permission`, `lsp-pi`, `golangci-lint`, `jina-web`

#### `permission`

Это встроенный extension из `pi-core`, который управляет тем, какие действия агент может выполнять без дополнительного подтверждения.

Уровни доступа:

- `minimal` — read-only сценарии;
- `low` — операции с файлами;
- `medium` — обычные dev-операции;
- `high` — почти полный доступ;
- `bypassed` — обход permission checks.

Что важно:

- dangerous commands вроде `sudo`, `rm -rf`, `chmod 777`, `dd`, `mkfs` всё равно считаются опасными;
- уровень можно менять прямо в сессии;
- настройки сохраняются в `~/.pi/agent/settings.json`.
...
Этот extension не заменяет `nono`: `permission` управляет поведением агента, а `nono` ограничивает сам процесс на уровне окружения.
...
#### `handoff` + `session-query`

Что дают вместе:

- `handoff` переносит ключевой контекст в новую сессию;
- `session-query` позволяет потом адресно спросить у прошлой сессии, какие решения были приняты, какие файлы менялись и почему.

Это особенно полезно для длинных задач, когда нужно дробить работу на несколько тредов без потери контекста.
...
Важно не путать `nono` и permissions layer:

- **`permission` в `pi-core`** — встроенный слой правил для самого агента: какие действия разрешены без дополнительного подтверждения и на каком permission level работает сессия;
- **`pi-permissions`** — опциональный внешний пакет, если нужен явный allow/deny ruleset по tool calls;
- **`nono`** — слой системной изоляции процесса Pi: доступ к файловой системе, рантаймам, сетям и другим ресурсам окружения.

То есть permissions отвечают за **policy/approval на уровне действий агента**, а `nono` — за **изоляцию на уровне ОС/ядра и runtime environment**. Они не заменяют друг друга, а закрывают разные классы рисков.
